...
async function main(): Promise<void> {
  ...
  await check("12. RECEIVED / PARTIALLY_RECEIVED POs cannot be ordered or cancelled", async () => {
    for (const status of ["RECEIVED", "PARTIALLY_RECEIVED"] as const) {
      const id = await makePo(ADMIN);
      // Simulate Phase-2-shaped data directly on the THROWAWAY db only.
      ...
      assert.ok(!can.ok && new RegExp(status).test(can.error), !can.ok ? can.error : "");
    }
  });
  ...
  console.log(`\nPO action tests: ${passed} passed, ${failed} failed.`);
