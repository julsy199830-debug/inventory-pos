"use client";

import { deleteCustomer } from "./actions";

/**
 * Per-row trash button that deletes a customer by ID.
 *
 * `deleteCustomer` is a Server Action invoked through the `<form action=...>`
 * prop, so submission works with progressive enhancement (JS off → plain
 * POST). The confirm gate needs client JS, so this is a Client Component:
 * we intercept `onSubmit` to confirm; cancelling the event stops the POST.
 * On confirm, the action runs, deletes the row, and `revalidatePath` swaps
 * the page — the row vanishes on its own, so no local pending state is needed.
 *
 * Like `deleteSupplier`, the row's `id` travels as a hidden field here — the
 * action reads it via `load(formData, "id")`. Customer has no inbound
 * relations in the schema, so there is no foreign-key violation to guard.
 */
export default function DeleteCustomerButton({
  id,
  name,
}: {
  id: string;
  /**
   * Optional display name for the confirm prompt and aria-label. The server
   * action keys off `id` (the hidden field below), so `name` is cosmetic —
   * the callers in `page.tsx` omit it and the button still deletes by `id`.
   */
  name?: string;
}) {
  const label = name ?? id;
  return (
    <form
      action={deleteCustomer}
      onSubmit={(e) => {
        if (!window.confirm(`Delete "${label}"?`)) {
          e.preventDefault();
        }
      }}
      className="inline-block"
    >
      <input type="hidden" name="id" value={id} />
      <button
        type="submit"
        aria-label={`Delete ${label}`}
        title={`Delete ${label}`}
        className="inline-flex items-center justify-center rounded-md p-1.5 text-zinc-400 transition-colors hover:bg-red-50 hover:text-red-600"
      >
        <svg
          className="h-4 w-4"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.8}
          stroke="currentColor"
          aria-hidden
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .567c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0C10.39 2.5 9.48 3.484 9.48 4.664v.916m7.5 0a48.523 48.523 0 0 0-7.5 0"
          />
        </svg>
      </button>
    </form>
  );
}
