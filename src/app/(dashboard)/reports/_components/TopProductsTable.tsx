 ...
 * falls back to an indigo-gradient initials avatar. `imageUrl` is typed on the
              ...
              {products.map((product, i) => (
                <tr key={product.productId} className="hover:bg-slate-50">
                  <td className="px-5 py-3">
                    <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-indigo-50 text-xs font-semibold text-indigo-700 ring-1 ring-indigo-100">
                      {i + 1}
                    </span>
                  </td>
                  <td className="px-5 py-3">
...
function ProductThumb({ product }: { product: AnalyticsTopProduct }) {
  ...
  return (
    <span
      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-indigo-700 text-xs font-semibold text-white"
      style={
        product.imageUrl
          ? {
